...
# ── Update eval/tasks.json with actual numbers ────────────────────────────

SCRIPT_DIR="$(cd "$(dirname "$0")" && pwd)"
TASKS_FILE="${SCRIPT_DIR}/eval/tasks.json"

if [[ -f "$TASKS_FILE" ]] && command -v jq &>/dev/null; then
  echo ""
  echo "==> Updating eval/tasks.json with actual issue/PR numbers..."

  jq \
    --arg otlp "${OTLP_ISSUE_NUM}" \
    --arg attrs "${ATTRS_ISSUE_NUM}" \
    --arg batch "${BATCH_ISSUE_NUM}" \
    --arg retry "${RETRY_ISSUE_NUM}" \
    --arg console "${CONSOLE_ISSUE_NUM}" \
    --arg typo "${TYPO_ISSUE_NUM}" \
    --arg grpc "${GRPC_ISSUE_NUM}" \
    --arg config "${CONFIG_ISSUE_NUM}" \
    --arg ci312 "${CI_312_ISSUE_NUM}" \
    --arg metrics "${METRICS_ISSUE_NUM}" \
    --arg unicode "${UNICODE_ISSUE_NUM}" \
    --arg pr1 "${PR1_NUM}" \
    --arg pr2 "${PR2_NUM}" \
    --arg pr3 "${PR3_NUM}" \
    '
    .tasks |= map(
      if .id == "T01" then
        .expected_output.explanation = "Open bugs: OTLP timeout (#\($otlp)), Batch shutdown (#\($batch)), Config crash (#\($config)), CI Python 3.12 (#\($ci312))."
      elif .id == "T02" then
        .description = "Who is assigned to issue #\($batch) (Batch exporter drops last batch on shutdown)?"
        | .expected_output.explanation = "Alice Chen is assigned to the batch exporter shutdown bug (#\($batch))."
      elif .id == "T03" then
        .expected_output.explanation = "PR #\($pr3) is the most recently created PR."
      elif .id == "T04" then
        .description = "Is issue #\($typo) (Typo in README.md) assigned to anyone?"
      elif .id == "T06" then
        .expected_output.explanation = "The batch shutdown bug has PR #\($pr1), the CI bug has PR #\($pr2). The OTLP timeout and config crash bugs have no PRs."
      elif .id == "T10" then
        .expected_output.explanation = "PR #\($pr1) has an approval from Bob. PRs #\($pr2) and #\($pr3) have no reviews."
      elif .id == "T13" then
        .description = "The word '\''recieve'\'' appears in README.md. Create a branch called `fix/readme-typo`, fix the typo, commit with an appropriate message, push, and open a PR that references issue #\($typo)."
        | .expected_output.checks[3] = "PR body or title references issue #\($typo)"
        | .expected_output.note = "Issue #\($typo) is the typo issue. Agent should reference it."
      elif .id == "T14" then
        .description = "Add the label `needs-triage` to issues #\($config) and #\($unicode)."
        | .expected_output.checks[1] = "Issue #\($config) has the '\''needs-triage'\'' label"
        | .expected_output.checks[2] = "Issue #\($unicode) has the '\''needs-triage'\'' label"
        | .expected_output.note = "Issue #\($config) is the config crash bug, #\($unicode) is the unicode bug."
      elif .id == "T15" then
        .description = "Close issue #\($typo) (the README typo) with a comment saying '\''Fixed in PR #X'\'' (use the actual PR number if one exists, otherwise note that no PR was found)."
        | .expected_output.checks[0] = "Issue #\($typo) is closed"
        | .expected_output.checks[1] = "Issue #\($typo) has a comment referencing a PR number or stating no PR exists"
      elif .id == "T16" then
        .description = "Add a review comment to PR #\($pr2) saying '\''LGTM — just verify the 3.12 tests pass on the ARM runner too.'\''"
        | .expected_output.checks[0] = "PR #\($pr2) has a review or comment containing the specified text"
      elif .id == "T17" then
        .description = "Create a new label called `performance` with color `#ff9900` and description '\''Performance-related issues'\'', then apply it to issue #\($retry)."
        | .expected_output.checks[2] = "Issue #\($retry) has the '\''performance'\'' label"
        | .expected_output.note = "Issue #\($retry) is the retry config enhancement."
      elif .id == "T21" then
        .expected_output.criteria[1] = "Reports review status: PR #\($pr1) approved, PR #\($pr2) no reviews, PR #\($pr3) no reviews (draft)"
      else . end
    )
    ' "$TASKS_FILE" > "${TASKS_FILE}.tmp" && mv "${TASKS_FILE}.tmp" "$TASKS_FILE"

  echo "  tasks.json updated with issue numbers: OTLP=#${OTLP_ISSUE_NUM}, batch=#${BATCH_ISSUE_NUM}, typo=#${TYPO_ISSUE_NUM}, config=#${CONFIG_ISSUE_NUM}, unicode=#${UNICODE_ISSUE_NUM}, retry=#${RETRY_ISSUE_NUM}"
  echo "  tasks.json updated with PR numbers: PR1=#${PR1_NUM}, PR2=#${PR2_NUM}, PR3=#${PR3_NUM}"
else
  echo ""
  echo "WARNING: Could not update eval/tasks.json (file not found or jq not installed)."
  echo "  Install jq or manually update issue/PR numbers in tasks.json."
fi
